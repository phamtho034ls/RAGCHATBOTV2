...
**Government AI Copilot** là hệ thống **trợ lý hành chính AI thực chiến** dành cho cán bộ Văn hóa – Xã hội (VHXH) cấp xã, xây dựng trên kiến trúc **Production-Grade RAG + Agent Tools**. Kết hợp **LLM qua OpenAI API**, **Qdrant vector search + PostgreSQL keyword search** (hybrid retrieval), **Redis caching**, **Cross-encoder reranking**, **GPU NVIDIA CUDA** tăng tốc embedding, và **Direct DB Lookup** cho tra cứu điều luật chính xác. Giao diện tiếng Việt, dark theme hiện đại, hỗ trợ streaming realtime.
...
- ✅ **RAG Intent Classifier** — tự nhận diện loại câu hỏi bằng **embedding + ngân hàng câu ví dụ** (scenario / legal_lookup / multi_article / need_expansion); bật/tắt qua `USE_INTENT_CLASSIFIER`, fallback regex khi lỗi
...
- ✅ **Hybrid Retrieval** — Qdrant semantic search + PostgreSQL keyword search → RRF merge → BAAI/bge-reranker-base
...
- ✅ **Answer validation** — kiểm tra grounding trước khi trả lời (`threshold=0.20`), bao gồm legal keyword detection, article reference extraction
...
│                    → RRF Merge → BAAI/bge-reranker-base → Top 5     │
├──────────────────────────────────────────────────────────────────────┤
...
Original Query
  ...
  → Hybrid Search:
      ...
      └── PostgreSQL keyword search ILIKE (top 20 candidates)
  ...
  → Answer Validation  (cosine sim ≥ 0.20, fallback nếu thấp hơn)
...
User query (VD: "Điều kiện kinh doanh thể thao" hoặc "Phát triển thể thao thành tích cao")
  ...
  └─ Default RAG path:
        ...
        │     ├─ vector_search + keyword_search (legal_domains filter) → RRF → rerank
...
- **Module:** `app/retrieval/reranker.py`, model mặc định `BAAI/bge-reranker-base` (CrossEncoder).  
- **Input:** `query` (câu hỏi) và `candidates` (danh sách passage, mỗi phần tử có `text_chunk`).  
- **Cách hoạt động:**  
  - Với mỗi cặp (query, passage), cross-encoder cho một **điểm relevance** (rerank_score).  
...
**Trạng thái:** Các mục 1–6 đã triển khai; thêm **RAG Intent Classifier** (mục 7) trong `intent_classifier.py` — tự nhận diện loại câu hỏi bằng embedding + ngân hàng câu ví dụ, bật/tắt qua `USE_INTENT_CLASSIFIER`.
...
│   ├── scripts/
│   │   └── fix_doc_numbers.py         # Migration: sửa doc_number cũ (PostgreSQL + Qdrant)
│   └── app/
...
│       │   ├── keyword_retriever.py    # PostgreSQL ILIKE keyword search
│       │   ├── hybrid_retriever.py     # RRF merge + doc_number resolve + direct article lookup
...
## 🗄️ Database Schema (PostgreSQL – 5 bảng chuẩn hoá)
...
| `title` | TEXT | Tiêu đề văn bản (lấy từ filename khi upload) |
| `document_type` | VARCHAR(50) | Loại: Luật, Nghị định, Thông tư, ... |
| `issuer` | VARCHAR(255) | Cơ quan ban hành |
...
vector_chunks: idx_vchunk_vector_id, idx_vchunk_doc_article (document_id, article_id),
               idx_vchunk_chunk_type (chunk_type)  ★ NEW
...
Ngoài regex, RAG có thể dùng **embedding + ngân hàng câu ví dụ** để nhận diện loại câu hỏi (`intent_classifier.py`). Bật/tắt qua `USE_INTENT_CLASSIFIER` (mặc định `true`).
...
- **Luồng:** `_get_rag_intent_flags(query)` trong `rag_chain_v2`: nếu bật classifier thì gọi `get_rag_intents()`, nếu lỗi thì fallback regex. Các flag dùng cho is_commune_query, use_multi, multi_article_conditions.
...
| Biến | Mặc định | Mô tả |
|------|----------|-------|
| `EMBEDDING_MODEL` | `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2` | Model embedding đa ngôn ngữ |
...
| `RERANKER_MODEL` | `BAAI/bge-reranker-base` | Model cross-encoder reranker |
| `RERANKER_DEVICE` | `cpu` | Device reranker (cpu để không tranh GPU) |
| `ANSWER_VALIDATION_THRESHOLD` | `0.20` | Ngưỡng cosine sim kiểm tra grounding |
...
| `EMBEDDING_DIM` | `384` | Dimension vector embedding |
...
| **Embedding** | sentence-transformers (`paraphrase-multilingual-MiniLM-L12-v2`) | ≥3.0 |
| **Relational DB** | PostgreSQL (async SQLAlchemy + asyncpg) | 16 / ≥2.0 / ≥0.29 |
| **Vector DB** | Qdrant | latest |
| **Cache** | Redis | 7 |
| **Keyword Search** | PostgreSQL ILIKE + scoring (BM25-style) | — |
| **Reranker** | CrossEncoder (`BAAI/bge-reranker-base`) | sentence-transformers |
...
| Số chiều vector | 384 |
| Hỗ trợ ngôn ngữ | 50+ (bao gồm tiếng Việt) |
| Max sequence | 128 tokens |
| Kích thước model | ~500 MB (cache tại `~/.cache/huggingface/`) |

### Startup Sequence

```
lifespan(app):
  1. init_postgres()           → Tạo PostgreSQL tables (documents, articles, clauses, vector_chunks, chat_logs)
  2. ensure_collection()       → Tạo/kiểm tra Qdrant collection (law_documents, cosine, 384-dim)
  3. warmup_embeddings()       → Load sentence-transformers model vào GPU/CPU
  4. warmup_reranker()         → Load BAAI/bge-reranker-base cross-encoder model
...
Query
  ...
  └─ [Không match / fallback] →
      ...
      → embed_query()                        : shape (1, 384), L2 normalize
      → retrieval/vector_retriever.py        : Qdrant search (top 20, filtered)
      → retrieval/keyword_retriever.py       : PostgreSQL ILIKE + scoring (top 20)
      → RRF merge (k=60)
      → retrieval/reranker.py               : BAAI/bge-reranker-base → article grouping
...
| **Cross-encoder reranking** | BAAI/bge-reranker-base cho precision cao hơn |
...
- **GPU:** Lần đầu chạy tải model embedding ~500MB. Đổi `EMBEDDING_DEVICE=cpu` trong `.env` nếu không có GPU NVIDIA.
...
- test_ninh_binh_tool.py — Unit test cho Ninh Bình search tool (huyện, du lịch, dân số, legal guard)
- test_ninh_binh_tool.py — Unit test Ninh Bình search tool (districts, tourism, population, legal guard)

Chạy pytest (nếu đã cài):
...
7. Thêm PostgreSQL full-text search (tsvector) thay thế ILIKE để tăng hiệu năng keyword search.
